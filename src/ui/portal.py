...
def main():
    ...
    logo_path = Path(__file__).parent / "static" / "LOGO-SUNU.png"
    st.sidebar.image(str(logo_path), width=160, use_container_width=False)
    st.sidebar.markdown(
        ...
        '<span style="font-size:0.75rem;color:#B3B8C2;display:block;">Portail Assurance Vie</span>'
        '</div>'
        '</div>'
        '</div>',
        unsafe_allow_html=True,
